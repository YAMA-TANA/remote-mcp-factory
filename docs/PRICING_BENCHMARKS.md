...
The quotas below are intentionally not simple copies of competitors. They use public competitor plans as anchors, then apply a lower ceiling where PicoSvc's implementation has a meaningful marginal Cloudflare cost (Browser Run, Dynamic Workers, Containers/Sandbox, D1 writes, or persistent storage).

## Published quotas

| Service | Free | Pico ($1/mo) | PicoPlus ($5/mo) | Main benchmark / rationale |
| --- | --- | --- | --- | --- |
| MCP | 1 Edge MCP, 5k requests, 20 builds, 0 Sandbox | 5 Edge MCP, 250k requests, 200 builds, 0 Sandbox | 25 MCP, 2 Sandbox slots, 1M requests, 1k builds | Dynamic Workers are cheap enough for the low tiers, but Sandbox fallback runs on Containers and can create material idle memory/disk cost. Free and Pico are therefore Edge-only; PicoPlus gets two Sandbox slots. |
...
| Mail | 1 route, 100 mails | 5 routes, 2k mails | 25 routes, 20k mails | Developer inbound-mail services commonly expose a small free allowance. Cloudflare inbound Email Routing is unlimited; Worker execution remains metered. |
...
- Workers Paid base: $5/month per account, including 10M Worker requests and 30M CPU-ms.
...
- Containers/Sandbox: 25 GiB-hours memory, 375 vCPU-minutes and 200 GB-hours disk included; overages are billed by active/provisioned resource time.
- D1 Paid: 25B rows read and 50M rows written included; overage $0.001/M reads and $1/M writes; first 5 GB storage included.
- R2 Standard: 10 GB-month, 1M Class A operations and 10M Class B operations included; then $0.015/GB-month, $4.50/M Class A and $0.36/M Class B, with no Internet egress charge.
- Email Routing inbound: unlimited; Worker execution is still billed as Workers usage.
...
- 5 always-used Dynamic Workers: `5 × 30 × $0.002 ≈ $0.30/month` before request/CPU overage.
- 20 always-used Dynamic Workers: `20 × 30 × $0.002 ≈ $1.20/month` before request/CPU overage.
- 25 always-used Dynamic Workers: `25 × 30 × $0.002 ≈ $1.50/month` before request/CPU overage.
- A continuously awake `lite` Sandbox has provisioned 256 MiB memory and 2 GB disk, so even before active CPU it can cost roughly $2/month at marginal overage rates. This is why $1 MCP plans are Edge-only.
...
- 1M D1 usage-counter writes after the shared included allocation: about $1.00. This is important for high-volume Flags/JSON-style endpoints because each metered request currently writes its usage counter.
...
PicoSvc currently uses Clerk Billing over Stripe. Clerk Billing charges 0.7% of billing volume plus the underlying Stripe payment-processing fee. A Japanese Stripe account's standard online card rate is currently 3.6% per successful payment. Therefore a domestic-card $1 charge has roughly 4.3% percentage fees before taxes, refunds, disputes or currency-conversion costs; charging USD into a JPY-settled account can add Stripe's currency-conversion fee. This makes a $1 plan economically possible, but the platform should prefer Bundle Pico whenever a user wants multiple services because one $5 checkout is operationally cleaner than five separate $1 subscriptions.
...
- MCP Free and Pico are Edge-only. PicoPlus can allocate up to two Sandbox-backed MCPs; existing Sandbox MCPs are rechecked after a billing downgrade.
