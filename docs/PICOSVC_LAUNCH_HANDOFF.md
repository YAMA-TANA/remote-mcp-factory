...
2. Deploy the **Worker** from this repository using `npx wrangler deploy`. A Cloudflare Pages UI deploy alone does **not** update the Screenshot, Fetch, RSS or Monitor API. Confirm `DB`, `BROWSER`, R2, Dynamic Worker and Sandbox bindings and the required secrets on the Worker.
...
| Fetch | With an authenticated account, fetch metadata and markdown from a normal public HTML page; verify nonempty content, redirects, charset and output shape. Use a controlled test endpoint to return an empty body, binary content, >2 MiB chunked response without Content-Length, and a stalled body; verify HTTP 422, 415, 413 and 504 respectively. Verify the request consumes the expected quota. | Unbounded memory growth, hung response, HTML treated as binary or binary treated as HTML, empty success, private redirect accepted. |
...
The URL guard now rejects IPv4-mapped IPv6, NAT64, 6to4 and Teredo address forms as well as direct private IP literals; the redirect wrapper revalidates each Location. **This is not comprehensive DNS rebinding protection.** Before treating arbitrary customer URLs as fully SSRF-safe, evaluate DNS resolution and egress restrictions in the actual Worker/Browser environment, including redirects handled internally by Browser Run. Run these tests against controlled endpoints, not third-party private services.
...
- Consolidate the old Fetch implementation in `utility-advanced.ts` with `fetch-reliable.ts` once the new handler passes production acceptance; the reliable handler currently takes routing precedence.
