...
const COPY = {
  en: {
    ...
    free: 'Free', freeBody: 'A small free allowance where the product offers one. Limits vary by service.',
    pico: 'Pico', picoBody: 'Per service, per month. The standard paid tier for small projects.',
  ...
  ja: {
    ...
    free: 'Free', freeBody: 'Free枠がある製品では、小規模な無料枠を利用できます。上限はサービスごとに異なります。',
    pico: 'Pico', picoBody: '1サービスあたり月額$1。小規模プロジェクト向けの標準有料プランです。',
  ...
  'zh-CN': {
    ...
    free: 'Free', freeBody: '提供 Free 档的产品可使用小额免费额度，具体限制因服务而异。',
    pico: 'Pico', picoBody: '每项服务每月 $1，适合小型项目的标准付费档。',
    ...
    <main>
      ...
      <section className="shell deploymentsSection">
        ...
        <div className="deploymentGrid">
          <article className="deployment"><div className="deploymentTop"><div><strong>{t.free}</strong><p>{t.freeBody}</p></div><span className="status ready">$0</span></div><div className="runtime"><span>{t.perService}</span><span>{t.perMonth}</span></div></article>
      ...
      <section className="shell deploymentsSection">
        ...
        <div className="deploymentGrid">
          <article className="deployment"><div className="deploymentTop"><div><strong>{t.bundlePico}</strong><p>{t.bundlePicoBody}</p></div><span className="status ready">${PICOSVC_PRICING.bundles.pico}{t.perMonth}</span></div><div className="runtime"><span>Pico</span><span>{t.allServices}</span></div></article>
          <article className="deployment"><div className="deploymentTop"><div><strong>{t.bundlePro}</strong><p>{t.bundleProBody}</p></div><span className="status ready">${PICOSVC_PRICING.bundles.pro}{t.perMonth}</span></div><div className="runtime"><span>PicoPlus</span><span>{t.allServices}</span></div></article>
        </div>
