...
export default function FormsManagementDetail({ resource, api, onClose, onChanged, copyValue }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    try { await Promise.all([loadSettings(!dirty), loadSubmissions(appliedSearch), loadDeliveries()]); }
    catch (reason) { setError(errorMessage(reason)); }
    finally { setLoaded(true); setBusy(''); }
  }, [loadSettings, loadSubmissions, loadDeliveries, dirty, appliedSearch]);
  ...
  return <section className="formsConsole" aria-label={t.title}>
    ...
    <div className="formsConsoleGrid"><form className="formsConsoleCard formsConsoleSettings" onSubmit={event => { void save(event); }}><h3>{t.settings}</h3><p className="formsConsoleHint">{t.protect}</p><fieldset disabled={Boolean(busy) || !loaded}>
      ...
      <div className="formsConsoleActions"><button type="submit" disabled={!dirty}>{busy === 'save' ? t.loading : t.save}</button><button type="button" disabled={Boolean(busy)} onClick={() => { if (!dirty || window.confirm(t.discard)) void loadSettings(true).catch(reason => setError(errorMessage(reason))); }}>{t.reload}</button></div>
    </fieldset></form>
    <div className="formsConsoleSide"><section className="formsConsoleCard"><div className="formsConsoleSectionHead"><h3>{t.submissions} <small>{submissions.length}</small></h3><button type="button" disabled={Boolean(busy)} onClick={() => { void refresh(); }}>{t.refresh}</button></div>
