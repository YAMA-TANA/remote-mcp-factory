...
export default function ShotWorkspace() {
  ...
  async function capture(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await request('/api/picosvc/shot', {
        ...
        body: JSON.stringify({ url, format, width: Number(width), height: Number(height), fullPage, waitUntil, waitMs: Number(waitMs), ...(selector.trim() ? { selector: selector.trim() } : {}) }),
      });
      ...
      fileRef.current = next; setFileUrl(next);
  ...
  async function revoke(key: KeyInfo) {
    ...
    try { await request(`/api/picosvc/shot/keys/${key.id}`, { method: 'DELETE' }); await listKeys(); }
    catch (reason) { setKeyError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setKeyBusy(false); }
  }
