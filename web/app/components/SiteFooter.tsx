...
const COPY = {
  en: { tagline: 'Small developer services. One account, only the tools you need.', products: 'Explore', support: 'Information', pricing: 'Pricing', dashboard: 'My dashboard', docs: 'Customer documentation', built: 'Developer tools, thoughtfully sized.' },
  ja: { tagline: '必要な開発者サービスを、ひとつのアカウントで。', products: 'サービス', support: 'ご案内', pricing: '料金', dashboard: 'マイダッシュボード', docs: '使い方・ドキュメント', built: '必要な分だけ使える開発者ツール。' },
  'zh-CN': { tagline: '一个账号，按需使用开发者服务。', products: '浏览服务', support: '更多信息', pricing: '价格', dashboard: '我的总览', docs: '使用指南', built: '按需使用的开发者工具。' },
} as const;
    ...
    <footer className="picoFooter" aria-label="PicoSvc footer">
      <div className="shell">
        <div className="picoFooterGrid">
          ...
          <nav className="picoFooterColumn" aria-label={t.support}><strong>{t.support}</strong><a href={localizedHref('/docs')}>{t.docs} →</a><a href={localizedHref('/pricing')}>{t.pricing}</a><a href={localizedHref('/contact')}>{c.contact} / {c.support}</a><a href={localizedHref('/terms')}>{c.terms}</a><a href={localizedHref('/privacy')}>{c.privacy}</a><a href={localizedHref('/tokushoho')}>{c.commercial}</a></nav>
        </div>
