...
type Preview = { text?: string; hash?: string; source?: string; monthlyChecksUsed?: number; options?: unknown };
const DEFAULT_OPTIONS: Options = { enabled: true, contentSelector: '', ignoreSelector: '', stripPattern: '' };
const TEXT = {
  en: { title: 'Monitor health & diagnostics', intro: 'Inspect fetch errors and changes. Test your selectors before saving them.', choose: 'Select a monitor', refresh: 'Refresh status', never: 'Not checked yet', last: 'Last checked', status: 'Status', working: 'Tracking', failed: 'Fetch failed', noMonitors: 'Create a monitor above to inspect it here.', noEvents: 'No change or fetch-error events yet. The first check runs on the scheduled worker.', events: 'Recent events', settings: 'Content detection', content: 'Content CSS selector (optional)', ignore: 'Ignore CSS selector (optional)', strip: 'Text exclusion pattern (optional)', save: 'Save detection settings', saving: 'Working…', reset: 'Saving changed settings resets the comparison baseline.', preview: 'Test current inputs (uses one check)', previewHelp: 'Tests the values currently in these fields without saving them or resetting change history.', ready: 'Extraction preview', result: 'Result', error: 'Could not load monitor diagnostics', saved: 'Settings saved; the next check will establish a new baseline.', disabled: 'Paused', unchanged: 'No unsaved changes', blank: 'No text extracted', drafts: 'Unsaved settings tested; they have not been applied to scheduled checks.' },
  ja: { title: 'Monitor の稼働状況・診断', intro: '取得失敗・変更履歴を確認し、保存前にCSSセレクターをテストできます。', choose: '監視対象を選択', refresh: '状態を更新', never: '未確認', last: '最終確認', status: '状態', working: '監視中', failed: '取得失敗', noMonitors: '上の画面でMonitorを作成すると、ここで診断できます。', noEvents: '変更・取得失敗の履歴はまだありません。初回確認は定期実行時に行われます。', events: '最近のイベント', settings: '検出条件', content: '監視するCSSセレクター（任意）', ignore: '除外するCSSセレクター（任意）', strip: 'テキスト除外パターン（任意）', save: '検出条件を保存', saving: '処理中…', reset: '変更を保存すると差分比較の基準がリセットされます。', preview: '現在の入力で抽出テスト（確認枠を1回使用）', previewHelp: '入力中の値を保存せずにテストします。監視設定と差分履歴は変更しません。', ready: '抽出プレビュー', result: '結果', error: '診断情報を取得できませんでした', saved: '設定を保存しました。次回の確認で比較基準を作成します。', disabled: '停止中', unchanged: '未保存の変更はありません', blank: 'テキストを抽出できませんでした', drafts: '未保存の設定でテストしました。定期監視にはまだ反映されていません。' },
  'zh-CN': { title: '监控状态与诊断', intro: '查看抓取错误与变更历史，保存前先测试 CSS 选择器。', choose: '选择监控', refresh: '刷新状态', never: '尚未检查', last: '最近检查', status: '状态', working: '监控中', failed: '抓取失败', noMonitors: '先在上方创建监控，即可在这里查看诊断。', noEvents: '尚无变更或抓取失败记录。首次检查将在定时任务运行时执行。', events: '最近的事件', settings: '内容检测', content: '内容 CSS 选择器（可选）', ignore: '忽略 CSS 选择器（可选）', strip: '文本排除模式（可选）', save: '保存检测设置', saving: '处理中…', reset: '保存更改将重置内容比较基线。', preview: '测试当前输入（消耗一次检查额度）', previewHelp: '仅测试输入框中的值，不会保存设置或重置变更历史。', ready: '提取预览', result: '结果', error: '无法获取监控诊断', saved: '设置已保存；下次检查将建立新基线。', disabled: '已暂停', unchanged: '没有未保存的更改', blank: '没有提取到文本', drafts: '已测试未保存设置，定时监控尚未使用这些设置。' },
} as const;
...
export default function MonitorDiagnostics() {
  ...
  const [savedOptions, setSavedOptions] = useState<Options>(DEFAULT_OPTIONS);
  const [error, setError] = useState('');
  const [notice, setNotice] = useState('');
  const [preview, setPreview] = useState<Preview | null>(null);
  const [busy, setBusy] = useState(false);
  const current = monitors.find(item => item.id === id);
  const dirty = JSON.stringify(options) !== JSON.stringify(savedOptions);
  ...
  useEffect(() => {
    ...
    let active = true;
    setEvents([]); setPreview(null); setNotice('');
    Promise.all([
      ...
      const loaded = { enabled: config.enabled !== false, contentSelector: config.contentSelector || '', ignoreSelector: config.ignoreSelector || '', stripPattern: config.stripPattern || '' };
      setEvents(Array.isArray(records.events) ? records.events : []);
      setOptions(loaded); setSavedOptions(loaded);
    }).catch(reason => { if (active) setError(reason instanceof Error ? reason.message : String(reason)); });
    return () => { active = false; };
  }, [id, request, signedIn]);

  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!id || busy || !dirty) return;
    ...
    try {
      ...
      setSavedOptions({ ...options }); setPreview(null);
  ...
  async function testExtraction() {
    ...
    try {
      const result = await request(`/api/picosvc/monitor/${encodeURIComponent(id)}/preview`, {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ contentSelector: options.contentSelector, ignoreSelector: options.ignoreSelector, stripPattern: options.stripPattern }),
      }) as Preview;
      setPreview(result);
      if (dirty) setNotice(t.drafts);
    } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setBusy(false); }
  }
