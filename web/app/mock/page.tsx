...
export default function MockPage() {
  ...
  async function refresh() {
    if (!signedIn) return;
    try {
      setData(await api<EndpointList>('/api/picosvc/mock/endpoints'));
      setMessage('');
    } catch (error) {
  ...
  async function createEndpoint() {
    ...
    setMessage('');
    try {
      await api('/api/picosvc/mock/endpoints', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ name, method, path, statusCode: Number(statusCode), body }),
      });
      await refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : String(error));
  ...
  async function removeEndpoint(id: string) {
    setBusy(true);
    try {
      await api(`/api/picosvc/mock/endpoints/${id}`, { method: 'DELETE' });
      await refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : String(error));
    } finally {
  ...
  return (
    <main>
        ...
        {!signedIn ? (
          ...
          <div className="deployCard">
            ...
            <div className="options">
              ...
              <label>Path <input value={path} onChange={(event) => setPath(event.target.value)} placeholder="hello" /></label>
              <label>Status <input value={statusCode} onChange={(event) => setStatusCode(event.target.value)} inputMode="numeric" /></label>
            </div>
            <label>Response body</label>
            <textarea value={body} onChange={(event) => setBody(event.target.value)} rows={8} style={{ width: '100%', marginTop: 8, padding: 14, borderRadius: 10, fontFamily: 'monospace' }} />
            <div style={{ marginTop: 14 }}><button className="primary" disabled={busy || !name.trim()} onClick={createEndpoint}>{busy ? 'Saving…' : 'Create endpoint'}</button></div>
      ...
      <section className="shell deploymentsSection">
        <div className="sectionHead"><div><span className="kicker">YOUR MOCKS</span><h2>Endpoints</h2></div>{signedIn && <button className="ghost" onClick={() => void refresh()}>Refresh</button>}</div>
        {!signedIn ? <div className="empty">Sign in to view endpoints.</div> : data.endpoints.length === 0 ? <div className="empty">No mock endpoints yet.</div> : (
          <div className="deploymentGrid">
            {data.endpoints.map((endpoint) => (
              <article className="deployment" key={endpoint.id}>
                <div className="deploymentTop"><div><strong>{endpoint.name}</strong><p>{endpoint.method} · {endpoint.statusCode}</p></div><span className={`status ${endpoint.enabled ? 'ready' : ''}`}>{endpoint.enabled ? 'active' : 'disabled'}</span></div>
                <div className="success"><code style={{ overflowWrap: 'anywhere' }}>{endpoint.endpoint}</code></div>
                <div className="deploymentBottom"><button className="ghost" disabled={busy} onClick={() => navigator.clipboard.writeText(endpoint.endpoint)}>Copy URL</button><button className="ghost" disabled={busy} onClick={() => void removeEndpoint(endpoint.id)}>Delete</button></div>
              </article>
            ))}
          </div>
        )}
