...
export default function CustomerProductPage({ service, locale }: { service: ProductSlug; locale: Locale }) {
  ...
  return <main className="customerPage">
    ...
    <div className="shell customerBody">
      ...
      {quota && <section className="customerSection"><h2>{c.pricing}</h2><div className="customerPlans"><article><strong>{c.freeTier} · $0</strong><p>{formatQuota(quota.free, locale)}</p></article><article><strong>{c.picoTier}</strong><p>{formatQuota(quota.pico, locale)}</p></article><article><strong>{c.picoPlusTier}</strong><p>{formatQuota(quota.picoPlus, locale)}</p></article></div><p className="customerFine">{c.quota}</p></section>}
      <ServicePurchasePlans service={service} locale={locale} />
