...
type StudioCopy = { heading: string; intro: string; first: string; second: string; third: string; action: string; item: string; empty: string };
...
const DESCRIPTIONS: Record<StudioSlug, Record<Language, StudioCopy>> = {
  mcp: {
    en: { heading: 'Deploy a remote MCP server', intro: 'Choose a GitHub repository, branch and endpoint access. Manage credentials after creation.', first: 'Repository', second: 'Deployment settings', third: 'Remote endpoints', action: 'Deploy server', item: 'Deployment', empty: 'No servers yet. Start with a GitHub repository.' },
    ...
    'zh-CN': { heading: '部署远程 MCP 服务', intro: '选择 GitHub 仓库、分支和访问方式。部署后管理密钥与端点。', first: '仓库', second: '部署设置', third: '远程端点', action: '部署 MCP', item: '部署', empty: '还没有服务器。请先输入 GitHub 仓库。' },
  },
  rss: {
    en: { heading: 'Turn a page into an RSS feed', intro: 'Start with the source page. Fine-tune article selectors only when extraction needs help.', first: 'Source page', second: 'Article extraction', third: 'Published feeds', action: 'Create feed', item: 'Feed', empty: 'No feeds yet. Add a public page to begin.' },
  ...
  mail: {
    en: { heading: 'Forward incoming mail to a webhook', intro: 'Create a route, then copy its inbound address and inspect delivery events.', first: 'Incoming route', second: 'Delivery destination', third: 'Mail routes', action: 'Create mail route', item: 'Mail route', empty: 'No mail routes yet.' },
    ...
    'zh-CN': { heading: '将来信转发到 Webhook', intro: '设置转发目标，然后获取收件地址并查看投递记录。', first: '邮件路由', second: '转发目标', third: '邮件路由', action: '创建路由', item: '路由', empty: '还没有邮件路由。' },
  },
  qr: {
  ...
  cron: {
    en: { heading: 'Schedule an HTTP request', intro: 'Choose a UTC schedule, method and destination. Add headers or a body when required.', first: 'Schedule (UTC)', second: 'HTTP request', third: 'Scheduled jobs', action: 'Schedule job', item: 'Job', empty: 'No scheduled jobs yet.' },
  ...
  functions: {
    en: { heading: 'Publish an edge function', intro: 'Name a function and write its JavaScript fetch handler. Edit source revisions after deployment.', first: 'Function identity', second: 'JavaScript editor', third: 'Deployed functions', action: 'Publish function', item: 'Function', empty: 'No functions yet.' },
  ...
  json: {
    en: { heading: 'Create a JSON document store', intro: 'Create a private store, save its one-time token, then read and edit documents.', first: 'Store name', second: 'Private document access', third: 'JSON stores', action: 'Create store', item: 'Store', empty: 'No stores yet.' },
  ...
  files: {
    en: { heading: 'Create a space for your files', intro: 'Organize uploads into a space. Upload and inspect individual objects in its management panel.', first: 'File space', second: 'Upload workflow', third: 'File spaces', action: 'Create space', item: 'Space', empty: 'No file spaces yet.' },
  ...
  flags: {
    en: { heading: 'Manage feature flags by project', intro: 'Separate environments into projects. Add and edit flags in the project panel.', first: 'Application environment', second: 'Configuration workflow', third: 'Configuration projects', action: 'Create project', item: 'Project', empty: 'No configuration projects yet.' },
  ...
  monitor: {
    en: { heading: 'Watch a page for changes', intro: 'Choose a public page and check interval. Add an optional webhook for notifications.', first: 'Page to monitor', second: 'Check frequency & alerts', third: 'Page monitors', action: 'Create monitor', item: 'Monitor', empty: 'No monitors yet.' },
  ...
  forms: {
    ...
    'zh-CN': { heading: '创建表单接收端点', intro: '无需自建后端即可接收提交，创建后查看地址和记录。', first: '表单名称', second: '接收流程', third: '表单', action: '创建表单', item: '表单', empty: '还没有表单。' },
  },
};
const UI = {
  en: { back: 'All services', pricing: 'Pricing', create: 'Create', resources: 'Resources', refresh: 'Refresh', loading: 'Loading…', signIn: 'Sign in to PicoSvc', signInHelp: 'Sign in to create and manage your resources.', noConfig: 'API or Clerk configuration is missing.', failed: 'Operation failed:', created: 'Created', close: 'Close', copy: 'Copy', copied: 'Copied', oneTime: 'Save this one-time credential', hide: 'Hide', show: 'Show', search: 'Search by name, URL or ID', all: 'All', active: 'Active', paused: 'Paused', noMatch: 'No matching resources.', reset: 'Clear filters', manage: 'Manage', details: 'Resource details', hint: 'You can manage individual resources below.' },
  ja: { back: '全サービス', pricing: '料金', create: '新規作成', resources: 'リソース', refresh: '更新', loading: '処理中…', signIn: 'PicoSvcにログイン', signInHelp: 'ログインすると作成・管理できます。', noConfig: 'APIまたはClerkの環境変数が未設定です。', failed: '操作に失敗しました：', created: '作成しました', close: '閉じる', copy: 'コピー', copied: 'コピーしました', oneTime: '今回だけ表示される認証情報。安全に保存してください', hide: '隠す', show: '表示', search: '名前・URL・IDで検索', all: 'すべて', active: '稼働中', paused: '停止中', noMatch: '一致するリソースがありません。', reset: '絞り込みを解除', manage: '管理・編集', details: 'リソースの詳細', hint: '作成後は右側の一覧から個別に管理できます。' },
  'zh-CN': { back: '所有服务', pricing: '价格', create: '新建', resources: '资源', refresh: '刷新', loading: '处理中…', signIn: '登录 PicoSvc', signInHelp: '登录后创建和管理资源。', noConfig: 'API 或 Clerk 环境变量未配置。', failed: '操作失败：', created: '已创建', close: '关闭', copy: '复制', copied: '已复制', oneTime: '此凭证只显示一次，请安全保存', hide: '隐藏', show: '显示', search: '按名称、URL 或 ID 搜索', all: '全部', active: '运行中', paused: '已暂停', noMatch: '没有匹配的资源。', reset: '清除筛选', manage: '管理', details: '资源详情', hint: '创建后可从右侧列表管理各项资源。' },
} as const;
const JA_FIELDS: Record<string, string> = { repoUrl: 'GitHubリポジトリURL', branch: 'ブランチ', visibility: 'アクセス方法', name: '名前', sourceUrl: '取得元URL', itemSelector: '記事のCSSセレクター', titleSelector: 'タイトルのCSSセレクター', linkSelector: 'リンクのCSSセレクター', contentSelector: '本文のCSSセレクター', dateSelector: '日付のCSSセレクター', webhookUrl: 'Webhook URL', targetUrl: '転送先URL', cron: 'Cron式（UTC）', method: 'HTTPメソッド', headers: 'リクエストヘッダー（JSON）', body: 'リクエスト本文', code: 'JavaScriptコード', intervalMinutes: '確認間隔（分）' };
const ZH_FIELDS: Record<string, string> = { repoUrl: 'GitHub 仓库 URL', branch: '分支', visibility: '访问方式', name: '名称', sourceUrl: '来源 URL', itemSelector: '文章 CSS 选择器', titleSelector: '标题选择器', linkSelector: '链接选择器', contentSelector: '正文选择器', dateSelector: '日期选择器', webhookUrl: 'Webhook URL', targetUrl: '目标 URL', cron: 'Cron 表达式 (UTC)', method: 'HTTP 方法', headers: '请求头 (JSON)', body: '请求正文', code: 'JavaScript 代码', intervalMinutes: '检查间隔（分钟）' };
...
export default function ServiceStudio({ service }: { service: StudioSlug }) {
  ...
  function section(label: string, content: ReactNode, key: string) { return <div className="studioFormSection" key={key}><div className="studioSectionLabel">{label}</div>{content}</div>; }
  function composer() {
    switch (service) {
    ...
    <div className="shell studioMain">
      ...
      {!authReady ? <div className="studioSignIn" role="status">{t.loading}</div> : !signedIn ? <div className="studioSignIn"><h2>{t.signIn}</h2><p>{t.signInHelp}</p><button className="primary" type="button" disabled={!clerk} onClick={() => clerk?.openSignIn()}>{t.signIn}</button></div> : <>
        <div className="studioColumns"><section className="studioCard studioComposer" aria-label={t.create}><div className="studioCardHeading"><span>01 / {t.create}</span><h2>{copy.action}</h2></div><form onSubmit={event => { event.preventDefault(); void create(); }}><fieldset disabled={busy}>{composer()}<div className="studioSubmit"><button className="primary" type="submit">{busy ? t.loading : copy.action} →</button><small>{t.hint}</small></div></fieldset></form>
          {created && <div className="studioCreated" role="status"><div className="studioCreatedHead"><strong>✓ {t.created}</strong><button type="button" className="studioSecondary" onClick={() => { setCreated(null); setRevealed({}); }}>{t.close}</button></div>{Object.entries(created).map(([key, value]) => {
