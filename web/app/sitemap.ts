...
const PAGES: PageKind[] = ['home', 'mock', 'hooks', 'contact', 'terms', 'privacy', 'tokushoho'];

export default function sitemap(): MetadataRoute.Sitemap {
  return PAGES.flatMap((kind) => LOCALES.map((locale: Locale) => ({
    url: localizedUrl(locale, kind),
    changeFrequency: kind === 'home' || kind === 'mock' || kind === 'hooks' ? 'weekly' as const : 'monthly' as const,
    priority: kind === 'home' ? 1 : kind === 'mock' || kind === 'hooks' ? 0.9 : 0.5,
    alternates: {
      languages: {
