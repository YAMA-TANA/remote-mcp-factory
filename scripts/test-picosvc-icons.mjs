...
const serviceApp = read('web/app/[locale]/[service]/app/page.tsx');
const publicProduct = read('web/app/customer-product-page.tsx');
const workspaceHeader = read('web/app/customer-workspace-header.tsx');
...
assert.match(servicePage, /icons: \{ icon: `\/icons\/\$\{service\}\.svg`/, 'Public service pages need their own favicon');
assert.match(serviceApp, /className="picoProductTheme"/, 'Separate workspace pages retain product theme');
assert.match(publicProduct, /\/icons\/\$\{service\}\.svg/, 'Public product explanations show the service icon');
assert.match(workspaceHeader, /\/icons\/picosvc\.svg/, 'Workspace navigation uses the brand logo');
assert.match(mockPage, /icons\/mock\.svg/);
assert.match(hooksPage, /icons\/hooks\.svg/);
console.log('PicoSvc rich SVG icons OK: 16 standalone service assets, branded favicon, separate public/workspace integration.');
