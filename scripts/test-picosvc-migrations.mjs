...
const migrations = readdirSync(new URL('../migrations/', import.meta.url))
  ...
  .sort();
assert.ok(migrations.length >= 9, 'Expected all PicoSvc schema migrations 0010–0018');
for (const migration of migrations) {
  ...
  assert.equal(integrity.integrity_check, 'ok', `${migration}: SQLite integrity check failed`);
}
const columns = (table) => new Set(db.prepare(`PRAGMA table_info(${table})`).all().map((column) => column.name));
for (const table of ['rss_feeds', 'rss_entries', 'webhook_events', 'webhook_deliveries', 'mock_endpoints', 'mock_rules', 'mock_requests', 'qr_links', 'qr_scan_events', 'flag_projects', 'feature_flags', 'mcp_sandbox_active_minutes', 'mail_events', 'cron_runs', 'file_objects', 'json_documents']) {
...
assert.ok(columns('rss_feeds').has('selectors_json'), 'RSS selectors migration missing');
assert.ok(columns('flag_projects').has('version'), 'Remote Config version missing');
assert.ok(columns('qr_scan_events').has('country'), 'QR country analytics missing');
