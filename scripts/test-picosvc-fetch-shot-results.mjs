...
const json = JSON.stringify({ url: dangerous, format: 'png', fullPage: true, waitUntil: 'networkidle2', waitMs: 900 });
assert.ok(examples.curl.includes(shellQuote(json)), 'cURL payload shell quoted');
assert.ok(examples.javascript.includes(`body: JSON.stringify(${json})`), 'JavaScript uses a JSON literal, not interpolated quoted strings');
