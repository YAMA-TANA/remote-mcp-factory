...
// Browser-backed quotas remain request-based until measured browser-time metering ships.
...
export const PICOSVC_PRODUCTS: PicoSvcProduct[] = [
  product({ slug: 'mcp', name: 'PicoSvc MCP', role: 'Edge-first MCP hosting / stdio-to-Remote conversion', status: 'active', endpointHost: 'mcp.picosvc.com', tiers: tiers(
    ...
    { mcps: 5, sandboxMcps: 0, sandboxActiveMinutes: 0, requests: 75_000, builds: 200 },
    { mcps: 25, sandboxMcps: 2, sandboxActiveMinutes: 10_000, requests: 250_000, builds: 1_000 },
  ) }),
  ...
  product({ slug: 'rss', name: 'PicoSvc RSS', role: 'Multi-item Web to RSS extraction', status: 'active', endpointHost: 'rss.picosvc.com', tiers: tiers(
    ...
    { feeds: 100, checks: 40_000, requests: 250_000, refreshMinutes: 30, entriesPerFeed: 20 },
  ) }),
  ...
  product({ slug: 'functions', name: 'PicoSvc Functions', role: 'Tiny edge JavaScript functions', status: 'active', endpointHost: 'fn.picosvc.com', tiers: tiers(
    ...
    { functions: 5, invocations: 40_000 },
    { functions: 20, invocations: 250_000 },
  ) }),
  product({ slug: 'json', name: 'PicoSvc JSON', role: 'Token-protected JSON key/value API', status: 'active', endpointHost: 'json.picosvc.com', tiers: tiers(
    ...
    { stores: 10, documents: 1_000, storageBytes: 50 * 1024 * 1024, requests: 30_000 },
    { stores: 50, documents: 3_000, storageBytes: 500 * 1024 * 1024, requests: 150_000 },
  ) }),
