  ...
  if (request.method === 'GET' && url.pathname === '/api/picosvc/account') {
    ...
    try {
      const rows = await env.DB.prepare(`
        SELECT bundle, product, tier, source, active, updated_at FROM bundle_entitlements
        WHERE owner=? ORDER BY bundle, product
      `).bind(identity.ownerId).all();
      bundleEntitlements = rows.results || [];
    } catch { // Rolling deploy compatibility before migration 0008.
    }
    const usage = await env.DB.prepare(`
      SELECT product, metric, quantity, updated_at FROM product_usage_monthly
      WHERE owner=? AND month=? ORDER BY product, metric
    `).bind(identity.ownerId, monthKey()).all();
    return json({
      userId: identity.userId, orgId: identity.orgId, ownerId: identity.ownerId, month: monthKey(),
      billing: PICOSVC_BILLING_MODEL, entitlements: entitlements.results, bundleEntitlements, usage: usage.results,
    });
  }
  return null;
}
